...
import co.edu.sena.proyect2687351.util.DBConnection;
...
public class UserRepositoryImpl  implements Repository<User>{
    ...
    public  List<User> listAllObj() throws SQLException {
        sql = "select u.user_id, u.user_firstname, u.user_lastname, u.user_email, u.user_password " + "from user_tbl u order by u.user_firstname, u.user_lastname";
    ...
    public Integer saveObj(User user) throws SQLException {
        ...
        if (user.getUser_id()!=null && user.getUser_id()>0){
            sql = "update user_tbl set user_firstname=?, user_lastname=?,user_email=?," + "user_password =aes_encrypt(?,)" +"where user_id=?";
        } else {
            sql="insert into user_tbl (user_firstname, user_lastname, user_email, user_password)" + "values(upper(?), upper(?), lower(?), "+ "aes_encrypt(?,'$2a$12$AxY05V.J8VognZln2uvqNegJ3N6lTu3iZhaoecb7JTQczsUEL6uB'))";
        }
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)){
